...
PALETTE = {
    ...
    "Rule-Based": "#2F855A",
    "Profit-Greedy Planner": "#2B6CB0",
    "Balanced Planner": "#C2410C",
    "Replay Q-Learning": "#7C3AED",
}
...
def plot_experiment(results: dict, output_path: str | Path) -> Path:
    """Create a three-panel plot from a saved experiment result dict."""
    ...
    ax = axes[0]
    if train_runs:
        curves = [np.array(run["train_rewards"], dtype=np.float64) for run in train_runs]
        ...
        color = PALETTE["Replay Q-Learning"]
        ax.plot(x, mean, color=color, lw=2, label="Replay Q-Learning")
...
def plot_svg_fallback(results: dict, output_path: str | Path) -> Path:
    """Dependency-free SVG visualization used when Matplotlib is unavailable."""
    ...
    parts = [
        ...
        '<text x="40" y="32" font-family="Arial" font-size="20" '
        'font-weight="700">RL Dynamic Pricing Results</text>',
        '<text x="40" y="70" font-family="Arial" font-size="15" '
        'font-weight="700">Gross profit per step</text>',
        '<line x1="40" y1="350" x2="420" y2="350" stroke="#111827"/>',
        '<line x1="40" y1="95" x2="40" y2="350" stroke="#111827"/>',
        '<text x="560" y="70" font-family="Arial" font-size="15" '
        'font-weight="700">Rate-acceptance trade-off</text>',
    ...
    for summary in summaries:
        ...
        label = summary.method.replace(" Planner", "").replace(" Pricing", "")
        parts.append(
    ...
    parts.extend(
        [
            '<text x="735" y="392" text-anchor="middle" font-family="Arial" '
            'font-size="12">Average offered rate (%)</text>',
            '<text x="515" y="230" transform="rotate(-90 515 230)" '
            'text-anchor="middle" font-family="Arial" font-size="12">Acceptance rate</text>',
            "</svg>",
        ]
    )
